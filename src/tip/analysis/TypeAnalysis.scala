...
class TypeAnalysis(program: AProgram)(implicit declData: DeclarationData) extends DepthFirstAstVisitor[Unit] with Analysis[TypeData] {
  ...
  def visit(node: AstNode, arg: Unit): Unit = {
    ...
    node match {
      ...
      case as: AAssignStmt => {
        val rhs = as.right
        def mkrecord(write: FieldWrite) = RecordType(
          allFieldNames.map(fname => {
            if (fname == write.field()) VarType(rhs)
            else FreshVarType()
          }))
        as.left match {
          case id: AIdentifier => unify(id, rhs)
          case w: ADerefWrite => unify(w.exp, PointerType(rhs))
          case w: ADirectFieldWrite => unify(w.id, mkrecord(w))
          case w: AIndirectFieldWrite => unify(w.exp, mkrecord(w))
        }
      }
      case bin: ABinaryOp =>
        bin.operator match {
      ...
      case call: ACallFuncExpr => {
        val rettype = FreshVarType()
        unify(call.targetFun, FunctionType(call.args, rettype))
        unify(call, rettype)
      }
      case _: AReturnStmt =>
      case rec: ARecord =>
